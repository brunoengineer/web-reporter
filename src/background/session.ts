...
export const getEventsBytes = async (): Promise<number> => {
  return chrome.storage.local.getBytesInUse(EVENTS_KEY);
};

export const startSession = (input: StartPayload): Promise<SessionMeta> =>
  serialize(async () => {
...
export const stopSession = (): Promise<SessionMeta | null> =>
  serialize(async () => {
...
export const appendEvents = (events: SessionEvent[]): Promise<number> =>
  serialize(async () => {
